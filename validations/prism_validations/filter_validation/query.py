def get_query():
    ...
    EXIST_REV_LOGIC AS (
        SELECT
            ...
            STUFF((
                ...
                WHERE 
                (
                    (PF.PROJECTPOINTID = FORM_POINTS.ProjectPointID AND PF.ProjectID = FORM.ProjectID) OR 
                    (PF_METRIC.PointTypeMetricID = FORM_METRIC.PointTypeMetricID AND PF.ProjectID = PARENT.ProjectID)
                ) 
                AND PF.FILTERID NOT IN (SELECT PF2.TemplateParentFilterID FROM prismdb.dbo.PointFilters PF2 WHERE PF2.ProjectID = FORM.ProjectID) FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 2, '') AS [FILTER],
    ...
    NON_EXIST_REV_LOGIC AS (
        SELECT
            PROJECT_ASSET.Description AS [ASSET], 'PROJECT' AS [FORM TYPE], IIF(PROJECT.DeployedProfileID IS NULL, 'N', 'Y') AS [DEPLOYED], MissingMetrics.ProjectName AS [FORM NAME], TEMPLATE.Name AS [PARENT TEMPLATE NAME], X.[INTERVAL TIME (SEC)], X.[POINT TYPE], 'N' AS [CONSTRAINED POINT], X.[METRIC NAME], X.[FUNCTION], '' AS [POINT NAME], '' AS [POINT DESCRIPTION], '' AS [POINT UNIT], X.[THRESHOLD TYPE], X.[FILTER], MissingMetrics.ProjectID AS [FORM ID], MissingMetrics.[METRIC ID] AS [METRIC ID]
        FROM (
    ...
    COMBINED_LOGIC AS (
        SELECT [FORM ID], [FORM NAME], [METRIC ID], [METRIC NAME], [POINT NAME], [POINT DESCRIPTION], [CONSTRAINED POINT], [FUNCTION], [FILTER], [FORM TYPE], [DEPLOYED], [THRESHOLD TYPE] FROM EXIST_REV_LOGIC
        UNION
        SELECT [FORM ID], [FORM NAME], [METRIC ID], [METRIC NAME], [POINT NAME], [POINT DESCRIPTION], [CONSTRAINED POINT], [FUNCTION], [FILTER], [FORM TYPE], [DEPLOYED], [THRESHOLD TYPE] FROM NON_EXIST_REV_LOGIC
    )
    SELECT
        x.[FORM NAME],
        x.[METRIC NAME],
